...
describe('Sites - Crawl Button', () => {
  ...
  it('should disable button while crawling', async () => {
    ...
    const crawlButton = screen.getByText('今すぐクロール') as HTMLButtonElement;
    fireEvent.click(crawlButton);

    await waitFor(() => {
      const button = screen.getByText('クロール中...').closest('button') as HTMLButtonElement;
...
describe('Sites - Crawl Result Link', () => {
  ...
  it('should render last crawled date as clickable link when crawl data exists', async () => {
    ...
    await waitFor(() => {
      const crawlDateButton = screen.getByText('2024/1/1 21:00:00');
  ...
  it('should render "未実施" for sites without crawl data', async () => {
    ...
    await waitFor(() => {
      const noCrawlText = screen.getByText('未実施');
      expect(noCrawlText).toBeInTheDocument();
    });
  });
  ...
  it('should show warning toast when clicking on site without crawl data', async () => {
    ...
    expect(screen.getByText('未実施').tagName).not.toBe('BUTTON');
  });
