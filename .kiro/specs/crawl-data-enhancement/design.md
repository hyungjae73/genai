...
本設計は、既存のWebクロールシステムに対して、スクリーンショット自動取得、構造化データ抽出、メタデータ取得、抽出データの永続化、レビューUI、価格履歴追跡機能を追加するものです。さらに、CrawlJobによるDB基盤のジョブ状態管理、HTML+OCRデュアル抽出、HTML vs OCR比較機能、抽出失敗時の目視確認機能を含みます。
...
6. **ジョブ状態の信頼性**: CrawlJobモデルによるDB基盤のジョブ状態管理で、Redis依存を排除
7. **デュアル抽出**: HTML構造解析とOCRの両方で抽出し、精度を向上
8. **抽出失敗時の対応**: 自動抽出が失敗した場合の目視確認・手動入力手段を提供
...
- **CrawlJob Manager**: DB基盤のクロールジョブ状態管理（pending/running/completed/failed）
- **OCR Engine**: スクリーンショットからのテキスト抽出
- **Comparison UI**: HTML抽出 vs OCR抽出の並列比較表示
- **Visual Confirmation UI**: 抽出失敗時のスクリーンショット・生HTML表示と手動入力
...
#### 責務
- クロールジョブの状態をデータベースで永続的に管理
- Redis/Celeryの結果バックエンドへの依存を排除
- 同一サイトの重複クロール防止（コンフリクト検出）
...
CREATE INDEX ix_crawl_jobs_site_id ON crawl_jobs(site_id);
CREATE INDEX ix_crawl_jobs_status ON crawl_jobs(status);
CREATE INDEX ix_crawl_jobs_celery_task_id ON crawl_jobs(celery_task_id);
```

#### API エンドポイント

```python
@router.post("/site/{site_id}", status_code=202)
async def start_crawl(site_id: int, db: Session = Depends(get_db)) -> CrawlJobResponse:
    """
    クロールジョブを開始。
    - サイト存在確認 → 404
    - pending/running ジョブ存在確認 → 409 Conflict
    - CrawlJob レコード作成 (status=pending)
    - Celery タスク送信、celery_task_id を保存
    """
    pass

@router.get("/status/{job_id}")
async def get_crawl_status(job_id: str, db: Session = Depends(get_db)) -> CrawlStatusResponse:
    """
    CrawlJob の状態をDBから取得して返す。
    completed → result を返却、failed → error_message を返却
    """
    pass
```
...
- Celeryタスク開始時: `pending` → `running`
- タスク正常完了時: `running` → `completed` + result JSONB + completed_at
- タスク失敗時: `running` → `failed` + error_message + completed_at
...
def _update_crawl_job_status(
    ...
    """
    CrawlJobのステータスを更新するユーティリティ。
    tasks.py内のCeleryタスクから呼び出される。
    """
    pass
```

### 11. デュアル抽出エンジン（HTML + OCR）

#### 責務
- HTML構造解析による支払い情報抽出
- スクリーンショットOCRによる支払い情報抽出
- 両方の抽出結果をsourceフィールドで区別して保存

#### ExtractedPaymentInfo モデル拡張

```python
class ExtractedPaymentInfo(Base):
    # ... 既存フィールド ...
    
    # 抽出ソース: "html" (HTML構造解析) or "ocr" (スクリーンショットOCR)
...
```mermaid
graph TB
    CrawlTask[Celery Crawl Task]
    
    subgraph "HTML Extraction"
        HTMLParse[HTML解析]
        HTMLExtract[ContentAnalyzer.extract_payment_info]
        HTMLSave[保存 source=html]
    end
    
    subgraph "OCR Extraction"
        Screenshot[スクリーンショット取得]
        OCRProcess[OCREngine.extract_text]
        OCRExtract[ContentAnalyzer.extract_payment_info]
        OCRSave[保存 source=ocr]
    end
    
    CrawlTask --> HTMLParse --> HTMLExtract --> HTMLSave
    CrawlTask --> Screenshot --> OCRProcess --> OCRExtract --> OCRSave
```

#### 実装詳細

1. **HTML抽出**: 既存のExtraction Engineを使用、source="html"で保存
2. **OCR抽出**:
   - `OCREngine.extract_text(screenshot_path)` でスクリーンショットからテキスト抽出
   - `ContentAnalyzer.extract_payment_info(ocr_text)` でOCRテキストから支払い情報抽出
   - source="ocr"で保存
3. **OCRメタデータ**: extraction_metadata JSONBに以下を格納:
   - `ocr_confidence`: OCR処理の信頼度
   - `screenshot_path`: 使用したスクリーンショットのパス
   - `ocr_text_length`: 抽出されたOCRテキストの文字数

### 12. HTML vs OCR 比較機能

#### 責務
- 同一クロール結果のHTML抽出とOCR抽出を並べて比較
- 抽出方法による差異の可視化
...
async def get_extracted_data_comparison(
    crawl_result_id: int,
    db: Session = Depends(get_db)
) -> dict:
    """
    HTML抽出データとOCR抽出データを取得して返す。
...
```typescript
// CrawlResultComparison.tsx
interface CrawlResultComparisonProps {
  siteId: number;
}

// compareMode: 'crawl' (クロール結果間比較) | 'source' (HTML vs OCR比較)
const CrawlResultComparison: React.FC<CrawlResultComparisonProps> = ({ siteId }) => {
  ...
  // 'source'モード: 選択したクロール結果のHTML抽出とOCR抽出を並べて表示
  // 'crawl'モード: 2つの異なるクロール結果を並べて表示
};
```

#### UI構成

- **比較モード切替トグル**: crawl（クロール結果間）/ source（HTML vs OCR）
- **サイドバイサイド表示**: 左にHTML抽出データ、右にOCR抽出データ
- **差異ハイライト**: 値が異なるフィールドを色分け表示
- **信頼度スコア表示**: 各抽出ソースの信頼度を並べて表示

### 13. 抽出失敗時の目視確認機能

#### 責務
- HTML抽出とOCR抽出の両方が失敗した場合の目視確認手段を提供
- スクリーンショットと生HTMLの表示
- 手動入力による契約条件の登録
...
```python
def is_extraction_failed(crawl_result_id: int, db: Session) -> bool:
    """
    HTML抽出とOCR抽出の両方が有効なデータを返さなかった場合にTrueを返す。
    
    判定条件:
    - 抽出レコードが存在しない
    - 抽出レコードのproduct_info, price_info, payment_methods, feesが全てnull/空
    """
    pass
```
...
async def get_visual_confirmation_data(
    crawl_result_id: int,
    db: Session = Depends(get_db)
) -> dict:
    """
...
async def save_manual_extraction(
    ...
    """
    目視確認後の手動入力データを保存。
    source="manual", confidence_score=1.0 で保存。
    """
    pass
```
...
├────────────────────────┴────────────────────────────┤
│  手動入力フォーム                                     │
│  商品名: [__________]  価格: [__________]            │
│  支払方法: [__________]  手数料: [__________]         │
│  [保存]                                              │
└─────────────────────────────────────────────────────┘
```
...
- **スクリーンショット表示**: CrawlResultのscreenshot_pathからスクリーンショットAPIで画像取得、ズーム・パン対応
- **生HTML表示**: CrawlResultのhtml_contentを取得、シンタックスハイライト表示、テキスト検索機能
- **手動入力**: 入力データはextracted_payment_infoテーブルにsource="manual"で保存
- **部分抽出時**: 抽出データが部分的に存在する場合は、抽出データと目視確認パネルを両方表示
