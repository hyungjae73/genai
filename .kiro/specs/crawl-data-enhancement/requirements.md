...
- **CrawlJob**: クロールジョブの状態をデータベースで管理するモデル（id, site_id, celery_task_id, status, result, error_message, created_at, completed_at）
- **OCR_Engine**: スクリーンショット画像からテキストを抽出するOCRエンジンコンポーネント
- **Content_Analyzer**: OCRテキストから支払い情報を抽出する分析コンポーネント
- **Comparison_UI**: HTML抽出データとOCR抽出データを並べて比較表示するUIコンポーネント
- **Visual_Confirmation_UI**: 抽出失敗時にスクリーンショットと生HTMLを表示し、目視確認を可能にするUIコンポーネント
...
### Requirement 26: CrawlJobによるクロールジョブ状態管理

**User Story:** 管理者として、クロールジョブの状態がデータベースで永続的に管理されることを期待する。これにより、Redis/Celeryの結果バックエンドに依存せず、信頼性の高いジョブ状態追跡ができる。

#### Acceptance Criteria

1. WHEN a crawl is initiated via POST /api/crawl/site/{site_id}, THE System SHALL create a CrawlJob record in the crawl_jobs table with status set to pending
2. THE CrawlJob SHALL store site_id, celery_task_id, status, result (JSONB), error_message, created_at, completed_at fields
3. WHILE a crawl task is executing, THE System SHALL update the CrawlJob status field to running
4. WHEN a crawl task completes successfully, THE System SHALL update the CrawlJob status to completed and store the result in the result JSONB field
5. WHEN a crawl task fails, THE System SHALL update the CrawlJob status to failed and store the error details in the error_message field
6. WHEN GET /api/crawl/status/{job_id} is called, THE System SHALL return the CrawlJob status and result from the database
7. WHEN a POST /api/crawl/site/{site_id} request is received and a pending or running CrawlJob exists for the same site_id, THE System SHALL return HTTP 409 Conflict
8. THE System SHALL create indexes on site_id, status, and celery_task_id columns of the crawl_jobs table

### Requirement 27: デュアル抽出（HTML + OCR）

**User Story:** 管理者として、クロール時にHTML構造解析とスクリーンショットOCRの両方で支払い情報が抽出されることを期待する。これにより、抽出精度を向上させ、HTML解析で取得できない情報をOCRで補完できる。

#### Acceptance Criteria

1. THE ExtractedPaymentInfo model SHALL include a source field (String(10), default="html") to distinguish extraction methods
2. WHEN a crawl is executed, THE Extractor SHALL first perform HTML-based extraction and save the result with source set to html
3. WHEN a crawl is executed, THE Extractor SHALL then perform OCR extraction from the screenshot and save the result with source set to ocr
4. THE OCR_Engine SHALL extract text from the screenshot image using OCR processing
5. THE Content_Analyzer SHALL extract payment information from the OCR text using the same extraction logic as HTML extraction
6. THE Extractor SHALL store OCR-specific metadata including ocr_confidence, screenshot_path, and ocr_text_length in the extraction_metadata JSONB field
7. WHEN screenshot capture fails, THE Extractor SHALL skip OCR extraction and log a warning
8. THE System SHALL store both HTML and OCR extraction records with the same crawl_result_id for later comparison

### Requirement 28: HTML vs OCR比較機能

**User Story:** レビュアーとして、HTML抽出データとOCR抽出データを並べて比較できることを期待する。これにより、抽出方法による差異を確認し、より正確なデータを選択できる。

#### Acceptance Criteria

1. THE API SHALL provide a GET endpoint at /api/crawl/extracted/{crawl_result_id}/compare to retrieve both HTML and OCR extraction records
2. THE API SHALL return a response containing html_data and ocr_data fields, each containing the full extracted payment information
3. WHEN only one extraction source exists for a crawl_result_id, THE API SHALL return null for the missing source
4. THE Comparison_UI SHALL display HTML extraction data and OCR extraction data side by side
5. THE Comparison_UI SHALL provide a compareMode toggle to switch between crawl result comparison mode and source comparison mode
6. THE Comparison_UI SHALL highlight fields where HTML and OCR extraction results differ
7. THE Comparison_UI SHALL display confidence scores for both HTML and OCR extractions to aid comparison

### Requirement 29: 抽出失敗時の目視確認機能

**User Story:** レビュアーとして、契約条件のテキスト抽出が失敗した場合（HTMLとOCRの両方）、UIでスクリーンショットと生HTMLを確認できることを期待する。これにより、自動抽出で取得できなかった契約条件を目視で確認できる。

#### Acceptance Criteria

1. WHEN both HTML extraction and OCR extraction return no payment information for a crawl result, THE Visual_Confirmation_UI SHALL display a visual confirmation mode
2. THE Visual_Confirmation_UI SHALL display the screenshot image of the crawled page for manual visual inspection
3. THE Visual_Confirmation_UI SHALL display the raw HTML content of the crawled page in a readable format
4. THE Visual_Confirmation_UI SHALL provide zoom and scroll functionality for the screenshot display
5. THE Visual_Confirmation_UI SHALL provide syntax-highlighted HTML view with search functionality for the raw HTML display
6. WHEN extraction data is partially available, THE Visual_Confirmation_UI SHALL display both the extracted data and the visual confirmation panel
7. THE Visual_Confirmation_UI SHALL allow the user to manually input extracted contract conditions after visual confirmation
8. THE Visual_Confirmation_UI SHALL store manually input data in the extracted_payment_info table with source set to manual and confidence_score set to 1.0
